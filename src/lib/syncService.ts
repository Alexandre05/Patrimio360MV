...
// 🚀 NOVIDADE: Guarda os "escutadores" para não travar o tablet com escutas repetidas
...
export function setupSync() {
  ...
  // Limpa as escutas antigas antes de ligar as novas (Economiza RAM do tablet)
  ...
  collections.forEach(({ name, dexie: table, pk }) => {
    ...
    // 🚀 NOVIDADE: Recua o relógio do tablet em 1 hora (3.600.000 ms) para "pescar" os itens que o Admin enviou enquanto a tela estava apagada
    const safeSyncTime = lastSyncTime > 3600000 ? lastSyncTime - 3600000 : 0;
    
    console.log(`[Sync Automático] Ligando antena para ${name}. Puxando desde: ${new Date(safeSyncTime).toLocaleTimeString()}`);
